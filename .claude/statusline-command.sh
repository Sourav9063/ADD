...
IFS=$'\x1f' read -r model_id effort_level ctx_used five_pct five_resets seven_pct seven_resets < <(
  cat | jq -r '[
    (.model.id // ""),
    (.effort.level // ""),
...
effort_short=$(echo "$effort_level" | awk '{print toupper(substr($0,1,1)) tolower(substr($0,2))}')
...
[ -n "$effort_short" ] && parts+=("$effort_short")
