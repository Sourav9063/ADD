...
Use a token scale — `dropdown 1000 / sticky 1100 / drawer 1200 / modal 1300 / popover 1400 / toast 1500` — and never write an ad-hoc `z-index: 9999`. Remember `z-index` only applies to positioned elements, and a parent's `transform`, `filter`, or `opacity` creates a stacking context that traps children no matter how high their value. When that bites, portal the overlay to the body instead of escalating numbers.
