...
Reuse existing UI primitives, typed state/variant contracts, and tokens before adding
custom controls. Visual scales and timings here and in companion skills are defaults;
the project's design system takes precedence. Accessibility contracts still apply.
...
its own hook. Component tokens reference semantic tokens, which reference primitives.
Reuse these instead of literals; justify local optical or content-driven exceptions.
...
Prefer short exits and reserve springs for meaningful attention. Favor `transform` and
`opacity`; profile necessary layout animation. Motion must not delay interaction.
...
- **Contrast**: 4.5:1 text, 3:1 large text (≥24px or ≥18.67px bold); 3:1 for visual information needed to identify controls and states. Inactive controls are exempt; keep them readable. `accessibility-audit` owns conformance checks.
- **Never signal with color alone.** ~1 in 12 people cannot rely on a red border. Pair color with an icon and text.
- **Focus**: never remove the outline without a visible replacement. Keep focus distinguishable from selection through shape or position, not color alone; a 2px ring with 2px offset is a useful default.
...
- **Hit targets**: aim for 44×44 CSS px on touch; WCAG 2.2 AA requires 24×24 or an applicable exception, including sufficient spacing.
...
- Test 200% text resizing and reflow at 320 CSS px; only content requiring two-dimensional layout may need horizontal scrolling.

Before handoff, tab through the surface and check token reuse and justified exceptions.
