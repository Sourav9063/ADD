...
Native `disabled` removes the button from tab order. Keep the reason visible rather than
depending on its tooltip. Use the existing primitive's disabled and busy contracts.
...
- Use native `disabled` for unavailable actions; use `aria-disabled` when focusability matters, with explicit guards against pointer, keyboard, and form activation.
- During loading, preserve the accessible name and focus, show busy state, and prevent duplicate submissions without removing the control.
...
- Guard in-flight submissions and restore operability on settlement. Debounce alone does not prevent duplicate writes; enforce replay safety server-side (`engineering`).
