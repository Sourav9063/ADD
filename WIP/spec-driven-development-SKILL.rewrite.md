...
- Verify before trusting. Treat every knowledge or plan claim as a lead; confirm it in code before acting on it.
- Code wins over docs. When a doc contradicts code, fix the doc in the same change. When the code looks wrong, report it and get approval before changing behavior.
...
`agents/plans/` holds execution state: goal, decisions, open questions, risks, ordered tasks, and verification checks. A plan steers work; it never defines current behavior. Record acceptance criteria in the plan's goal. Code and passing checks decide when a task is done.
